...
public sealed class SqlSelectSingleTest
{
    //--------------------------------------------------------------------------------
    // Other
    //--------------------------------------------------------------------------------
    ...
    public void TestSelectUpdate()
    {
        ...
        con.SetupCommand(static cmd =>
        {
            cmd.Executing = static c => Assert.Equal("SELECT * FROM MultiKey WITH (UPDLOCK, HOLDLOCK) WHERE Key1 = @p0 AND Key2 = @p1", c.CommandText);
            cmd.SetupResult(new MockDataReader(MultiKeyEntity.Columns, new List<object[]>()));
        });
