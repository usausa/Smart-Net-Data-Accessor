...
public sealed class SqlSelectTest
{
    ...
    public void TestSelectTop()
    {
        ...
        con.SetupCommand(static cmd =>
        {
            cmd.Executing = static c => Assert.Equal("SELECT TOP 10 * FROM MultiKey ORDER BY Key1, Key2", c.CommandText);
            cmd.SetupResult(new MockDataReader(MultiKeyEntity.Columns, new List<object[]>()));
        });
    ...
    public void TestSelectUpdate()
    {
        ...
        con.SetupCommand(static cmd =>
        {
            cmd.Executing = static c => Assert.Equal("SELECT * FROM MultiKey WITH (UPDLOCK, HOLDLOCK) ORDER BY Key1, Key2", c.CommandText);
            cmd.SetupResult(new MockDataReader(MultiKeyEntity.Columns, new List<object[]>()));
        });
