...
// PostgreSQL Builder の transform。共通解決（テーブル名・値パラメータ・列）は MethodResolver に委譲し、ここでは PostgresKind 別の
...
// MethodResolver; only the PostgresKind-specific model construction, diagnostics, and the Postgres-specific RETURNING
// column read happen here. Returns null when the method cannot be resolved.
internal static class PostgresModelBuilder
{
    public static BuilderMethodModel? BuildMethod(MethodBuildContext<PostgresKind> c)
    {
