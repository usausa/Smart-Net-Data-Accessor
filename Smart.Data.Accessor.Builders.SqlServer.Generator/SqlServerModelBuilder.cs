...
// SQL Server Builder の transform。共通解決（テーブル名・値パラメータ・列）は MethodResolver に委譲し、ここでは SqlServerKind 別の
...
// MethodResolver; only the SqlServerKind-specific model construction, diagnostics, and the SqlServer-specific OUTPUT
// column read happen here. Returns null when the method cannot be resolved.
internal static class SqlServerModelBuilder
{
    public static BuilderMethodModel? BuildMethod(MethodBuildContext<SqlServerKind> c)
    {
